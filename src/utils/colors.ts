/**
 * Color utility functions for converting HEX to OKLCH and generating palettes
 */
...
/**
 * Generate a 10-shade palette from a base color
...
export function generatePalette(
  ...
  // Define lightness values for each shade (light mode optimized)
  ...
  for (const [shade, lightness] of Object.entries(shades)) {
    ...
    if (lightness > 0.85 || lightness < 0.25) {
      chroma = base.c * 0.3;
    } else if (lightness > 0.75 || lightness < 0.35) {
      chroma = base.c * 0.6;
    } else if (lightness > 0.65 || lightness < 0.45) {
      chroma = base.c * 0.85;
    }

    palette[`--${prefix}-${shade}`] = formatOklch({
      l: lightness,
      c: Math.min(chroma, 0.15), // Cap chroma for better aesthetics
      h: base.h,
    });
  }
...
/**
 * Generate grayscale palette with a color tint
 */
export function generateGrayPalette(
  ...
  // Light mode grays
  const lightShades: Record<string, { l: number; c: number }> = {
    '50': { l: 0.99, c: 0.002 },
    '100': { l: 0.97, c: 0.003 },
    '200': { l: 0.93, c: 0.005 },
    '300': { l: 0.87, c: 0.008 },
  ...
  const darkShades: Record<string, { l: number; c: number }> = {
    '50': { l: 0.13, c: 0.003 },
    '100': { l: 0.17, c: 0.005 },
    '200': { l: 0.22, c: 0.008 },
    '300': { l: 0.30, c: 0.010 },
...
/**
 * Generate all CSS variables from branding colors
 */
export function generateThemeStyles(colors?: BrandingColors): string {
  ...
  let lightVars = Object.entries(primaryPalette)
    .map(([key, value]) => `${key}: ${value};`)
    .join('\n    ');

  lightVars += '\n    ' + Object.entries(grayPalettes.light)
    .map(([key, value]) => `${key}: ${value};`)
    .join('\n    ');

  // Build dark mode CSS
  let darkVars = Object.entries(primaryDarkPalette)
    .map(([key, value]) => `${key}: ${value};`)
    .join('\n    ');

  darkVars += '\n    ' + Object.entries(grayPalettes.dark)
    .map(([key, value]) => `${key}: ${value};`)
    .join('\n    ');

  // Generate primary shadows with new hue
  const baseOklch = hexToOklch(colors.primary);
  const shadowColor = formatOklch({ l: 0.55, c: 0.13, h: baseOklch.h });
  const shadowColorDark = formatOklch({ l: 0.65, c: 0.13, h: baseOklch.h });
